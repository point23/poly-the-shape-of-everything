...
export class Proximity_Grid {
    ...
    debug_render_grid(grid: Node) {
        const square_size = Const.Game_Board_Square_Size;
        const half_square_size = Const.Game_Board_Half_Square_Size;
        const origin_pos = Const.Game_Board_Orgin_Pos;

        const grid_pos_z = origin_pos.z - half_square_size + square_size * this.grid_size.width / 2;
        const grid_pos_x = origin_pos.x - half_square_size + square_size * this.grid_size.height / 2;
        const grid_pos_y = origin_pos.y - half_square_size;
        const grid_center = new Vec3(grid_pos_x, grid_pos_y, grid_pos_z);

        grid.setPosition(grid_center);
        grid.setScale(this.grid_size.height / 10, 1, this.grid_size.width / 10);

        const renderable = grid.getComponent(MeshRenderer);
        const mat = renderable.material;
        mat.setProperty('tilingOffset', new Quat(this.grid_size.height, this.grid_size.width, 0, 0));
    }
}
